...
class profile_index_action extends profile_controller
{
	...
	public function execute()
	{
		...
		$act = request::get('act');
		if(in_array($act, array('remove_profile', 'set_location', 'foreign_works', 'remove_foreign_work', 'to_reserv', 'from_reserv', 'add_smi', 'remove_smi')))
		{
			$this->set_renderer('ajax');
			return $this->json['success'] = $this->$act();
		}
		
		$this->page_position = array(
			"position" => 0,
			"page" => 0
		);
...
//				"reserv"=>0
		...
		$this->smi = user_smi_peer::instance()->get_list(array("user_id" => $this->user_id), array(), array("id DESC"));
		
		$this->hronologies = array();
		...
		foreach($this->albums as $category_key => $array)
		{
			...
			foreach($albums_id as $album_id)
			{
				...
				if(in_array($category_key, array('portfolio', 'photos')))
					continue;
