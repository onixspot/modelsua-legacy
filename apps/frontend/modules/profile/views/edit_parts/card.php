<style>
    ...
    .card_box div { position: relative;}
    .card_box div span{position: relative;}
</style>
<?$contacts = profile_peer::instance()->get_contacts($card_profile["user_id"]);?>
<?
    $agency = db::get_rows("SELECT * FROM user_agency WHERE (type=1 AND user_id=:uid) AND (agency_id>0 OR name<>'') LIMIT 1",array('uid'=>$card_profile['user_id']));
    if(empty($agency)) $agency = db::get_rows("SELECT * FROM user_agency WHERE (agency_id>0 OR name!='') AND user_id=:uid AND foreign_agency=false LIMIT 1",array('uid'=>$card_profile['user_id']));
    if(empty($agency)) $agency = db::get_rows("SELECT * FROM user_agency WHERE name!='' AND user_id=:uid AND foreign_agency=true LIMIT 1",array('uid'=>$card_profile['user_id']));
    
    
    if(!empty($agency)) 
	foreach ($agency as $key => $value) 
	    if($value['agency_id']>0) $agency = db::get_scalar("SELECT name FROM agency WHERE id=:aid",array('aid'=>$value['agency_id']));
	    else $agency = db::get_scalar("SELECT name FROM user_agency WHERE id=:aid",array('aid'=>$value['id']));
    else $agency = '';
?>
...
<div id="profile-edit-frame-card">
	...
	<tr>
	    ...
	    <td>
		<input type="text" id="email" value="<?=($contacts['email']) ? $contacts['email'] : ($card_profile['email'] ? $card_profile['email'] : 'example@sitename.domain'); ?>"/>
	    </td>
	</tr>
	<tr>
	    <td>
	    ...
	    <td style="padding-bottom: 10px;">
		<input type="text" id="www" value="<?=($card_profile['subdomain']) ? $card_profile['subdomain'] : 'example' ?>" style="width: 90px;"/>.<?=conf::get('server')?>
	    </td>
	</tr>
	<tr>
	    <td>&nbsp;</td>
    ...
    <div class="left mt10" style="margin-left: 35px;">
		...
		<img id="card_avatar" src="<? if($preview['id']){ ?>/imgserve?pid=<?=$preview['id']?><? } else { ?>/no_image.png<? } ?>" width="200" />
	</div>
        <div class="clear"></div>
	<div class="left mt5">
		...
		<div id="card_photo_crop" class="hide">
			<div class="mt10" style="border: 1px solid black; width: 100px; height: 138px;">
				<div id="card_avatar_small" style="width: 100px; height: 138px; overflow: hidden;">
					<img src="<? if($preview['id']){ ?>/imgserve?pid=<?=$preview['id']?><? } else { ?>/no_image.png<? } ?>" width="100px" height="138px" />
				</div>
			</div>
	...
	<div class="s2" style=""> 
	    <span id="email_span"><?=($contacts['email']) ? $contacts['email'] : ($card_profile['email'] ? $profile['email'] : 'example@sitename.domain'); ?></span>
	</div>
	<div class="s2" style="margin-top: 15px;"> 
	    <span id="www_span"><?=(($card_profile['subdomain']) ? $card_profile['subdomain'].'.'.conf::get('server') : 'example.'.conf::get('server')); ?></span>
	</div>
...
<script>
	...
	$.post(
	    '/profile/card',
	    {
		"data" : {  "name": preparePostData('name_span'),
