...
<table style="width: 100%" id="templates-table"> 
	...
	<?foreach($list as $num=>$Id) {
		...
		<td>
		    ...
		    <a href="javascript:;" onclick="$.post('/adminka/email_templates',{'delete': 1, 'del_id': '<?=$Id?>'},function(resp) { if(resp.success) $('#template-'+resp.id).remove();}, 'json')"><img src="/ui/delete.png"/></a>
		</td>
	    </tr>
	<? } ?>
    <? } ?>
