<div class="mt30 fs12">
	...
	<div>
		<div class="left square_p pl15 mb10 fs12 ucase bold">
			<a class="cblack" href='javascript:;'><?=t("СМИ")?></a>
		</div>
	...
	<? if(count($smi) > 0){ ?>
		...
		<? foreach($smi as $id){ ?>
			...
			<div id="smi-item-<?=$item['id']?>" class="pt10 pb5" style="<? if($cnt > 0){ ?>border-top: 1px solid #eee<? } ?>">
				<div>
					<? if(session::has_credential('admin') || session::get_user_id() == $user_id){ ?>
						<div class="fs10 pb5 aright">
							<a id="remove-smi-<?=$item['id']?>" class="fs10" href="javascript:;"><?=t('Удалить')?></a>
						</div>
					<? } ?>
					<div>
